package com.interview.array.SpaceRecycle;
...
public static void main(String[] args){
    ...
    //Most optimum I guess
    //Pending Still to practise
    public static  int firstMissingPositive(int[] A) {
    for(int i=0;i<A.length;i++){
        int correctPosition = A[A[i]-1];
        while(A[i]>0 && A[i]<=A.length && A[i]!=correctPosition){
            int temp=A[i];
            A[i]=A[correctPosition];
            A[correctPosition]=temp;
            correctPosition=A[A[i]-1];
        }
...
// this was using extra space we can also try without extra space
    public static  int firstMissingPositiveSpace(int[] A) {
        int count=0;
        for(int val:A){
            if(val>0){
                count++;
